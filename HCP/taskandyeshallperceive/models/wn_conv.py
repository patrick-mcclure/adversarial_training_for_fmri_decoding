...
class _Conv(base.Layer):
  ...
  def __init__(self, rank,
    ...
    self.sigma_initializer = tf.constant_initializer(0.001)#tf.random_normal_initializer(-4.5,0.01)
  ...
  def build(self, input_shape):
    ...
    g_shape = [1 for i in self.kernel_size] + [1, self.filters]
    ...
    self.g = self.add_variable(name='g',
                                    shape=g_shape,
                                 initializer=tf.constant_initializer(math.sqrt(1/16.0)),
                                    regularizer=None,
                                    constraint=None,
                                    trainable=True,
                                    dtype=self.dtype)   
    tf.summary.histogram(self.g.name,self.g) 
    self.v_norm = tf.nn.l2_normalize(self.v,[i for i in range(len(self.kernel_size)+1)])
    
    self.kernel = tf.multiply(self.g,self.v_norm,name = 'kernel') 
    tf.summary.histogram(self.kernel.name,self.kernel)
    tf.add_to_collection('kernels',self.kernel)
    ...
    if self.use_bias:
      ...
      tf.add_to_collection('biases',self.bias)
